-- insert_Reporttable1 프로시저를 삭제
-- 주석 처리하여 프로시저를 삭제.
-- DROP PROCEDURE insert_Reporttable1;
DELIMITER $$

-- insert_Reporttable1 프로시저를 생성
...
-- insert_Reporttable1 프로시저를 호출
CALL insert_Reporttable1();

-- Scoring 테이블에서 subjectID별로 score의 평균을 계산하여 조회
select subjectID, avg(score) as 평균
from Scoring
group by subjectID;
